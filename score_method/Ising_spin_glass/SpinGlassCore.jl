...
function metropolis_sampler(config, β, N, Jh, Jv)
    range_i = shuffle!(collect(1:N))
    range_j = shuffle!(collect(1:N))
    for i in range_i
        for j in range_j
            s = config[i, j]
            nb = Jv[i, j] * config[mod1(i + 1, N), j] + Jh[i, j] * config[i, mod1(j + 1, N)] +
                 Jv[mod1(i-1, N), j] * config[mod1(i - 1, N), j] + Jh[i, mod1(j-1, N)] * config[i, mod1(j - 1, N)]
            cost = 2s * nb
...
function calc_energy(config, N, Jh, Jv)
    energy = 0.0
    for i in 1:N, j in 1:N
        s = config[i, j]
        energy -= s * (Jh[i, j] * config[i, mod1(j + 1, N)] + Jv[i, j] * config[mod1(i + 1, N), j])
    end
    return energy
end
...
function score_method(
    ...
    for _ in 1:eqSteps
        for r in 1:M
            configs[r] = metropolis_sampler(configs[r], β_replicas[r], N, Jh, Jv)
        end
    end
    ...
    for step in 1:mcSteps
        for r in 1:M
            configs[r] = metropolis_sampler(configs[r], β_replicas[r], N, Jh, Jv)
            energy_sum[r] += calc_energy(configs[r], N, Jh, Jv)
        end
        ...
        if step % exchange_interval == 0
            ...
            for r in start:2:(M - 1)
                E1 = calc_energy(configs[r], N, Jh, Jv)
                E2 = calc_energy(configs[r + 1], N, Jh, Jv)
...
function calc_acceptance_and_rtt(
    ...
    for step in 1:mcSteps
        for i in 1:M
            ...
            configs[current_replica_idx] = metropolis_sampler(configs[current_replica_idx], β_replicas[i], N, Jh, Jv)
        end
        ...
        for r in start_idx:2:(M - 1)
            ...
            E1 = calc_energy(configs[rep_idx1], N, Jh, Jv)
            E2 = calc_energy(configs[rep_idx2], N, Jh, Jv)
...
export initial_state, metropolis_sampler, calc_energy, uniform_acceptance_grad_sm, calc_acceptance_and_rtt
